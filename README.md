# rotatefiles

高性能、生产级日志轮转组件

rotatefiles 是一个为生产环境设计的日志写入与轮换组件，支持：

* 基于时间周期自动切换日志文件
* 支持软链指向最新日志文件
* 支持异步写入，降低磁盘 IO 与写锁冲突
* 支持自动清理历史日志文件
* 支持 Flush 周期控制
* 支持同步关闭与优雅退出
* 支持自定义 Writer 包装（如缓冲写、加密写入等）
* 支持外部动态配置文件名生成

该组件可嵌入任意日志系统，尤其适用于：

* Web 服务大量高频日志写入
* AI 在线推理系统日志落盘
* 支持日志按分钟/小时/天切分
* 大规模边缘计算设备本地日志

---

---

## 📦 安装

```shell
go get github.com/lifei6671/rotatefiles
```

```go
package main

import (
	"context"
	"fmt"
	"io"
	"log"
	"path/filepath"
	"time"

	"github.com/lifei6671/rotatefiles"
)

func main() {
	filename, _ := filepath.Abs("./app.log")
	generator, err := rotatefiles.NewSimpleRotateGenerator("1min", filename, func(err error) {
		log.Println(err)
	})
	if err != nil {
		log.Fatalf("new rotatefiles failed: %s", err)
	}
	_ = generator.Start(context.Background())
	opt := &rotatefiles.RotateOption{
		RotateGenerator: generator,
		NewWriter: func(ctx context.Context, w io.WriteCloser) (rotatefiles.AsyncWriter, error) {
			return rotatefiles.NewAsyncWriter(w, 2014, rotatefiles.WithTimeout(time.Microsecond*10)), nil
		},
		FlushDuration: time.Second,
		MaxFileNum:    10,
	}

	rotateFile, err := rotatefiles.NewRotateFile(opt, rotatefiles.WithOnErr(func(err error) {
		log.Println(err)
	}))
	if err != nil {
		log.Fatalf("new rotatefile failed: %s", err)
	}
	defer rotateFile.Close()

	for i := 0; i < 500; i++ {
		timer := time.NewTicker(time.Second)
		go func() {
			for t := range timer.C {
				rotateFile.Write([]byte(fmt.Sprintf("%d - %d hello world\n", i, t.Unix())))
			}
		}()
		time.Sleep(time.Second * 5)
		timer.Stop()
	}

}

func init() {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
}

```

---

## ✨ 特性概览

| 能力               | 说明            |
| ---------------- | ------------- |
| 异步写入 AsyncWriter | 降低写入系统开销      |
| 周期 rotation      | 自动切换文件        |
| 软链管理             | 始终指向最新文件      |
| 自动清理旧文件          | 支持 MaxFileNum |
| 可插拔生成器           | 定义文件名逻辑       |
| 可插拔 Writer       | 自定义缓存模型       |
| 可观测性             | 支持状态读取        |

---

---

## 🔧 主要组件说明

### ① `RotateGenerator`

负责周期性产生日志目标信息：

```go
type RotateGenerator interface {
    Start(ctx context.Context) error
	Generate() RotateInfo
	Stop(context.Context) error
}
```

单次生成结构：

```go
type RotateInfo struct {
	Symlink  string // 软链，如 ./app.log
	Filename string // 原始日志，如 ./app.log
	FilePath string // 实际输出文件，如 ./app.log.202501021030
}
```

典型应用逻辑：

* 每小时生成一个新文件名
* 每天生成一个新文件名
* 手动远端配置动态推送新路径

---

### ② `PeriodicGenerator[T]`

用于定时刷新 RotateInfo

特性：

- ✔ 强一致周期保证
- ✔ 支持外部 context 中断
- ✔ 支持 retry
- ✔ 支持错误回调

示例：

```go
gen := NewPeriodicGeneratorFunc(
	func(ctx context.Context) (RotateInfo, error) {
		file := fmt.Sprintf("app.log.%s", time.Now().Format("200601021504"))
		return RotateInfo{Symlink: "app.log", FilePath: file}, nil
	},
	time.Minute,
)
```

---

### ③ `rotateFile`

核心 Writer 实现：

* Write 时写入异步 writer
* 轮换阶段自动切换/关闭/回收旧 Writer
* flush 定时执行
* Close 支持可等待式退出

---

---

## 🧱 架构流程图

```
                ┌───────────────────┐
                │ RotateGenerator   │
                │ 生成 FilePath      │
                └───────┬───────────┘
                        │周期触发
                        ▼
         ┌──────────────────────────────────┐
         │ PeriodicGenerator[RotateInfo]    │
         │ 缓存最新 FilePath                  │
         └───────┬──────────────────────────┘
                 │每次 flush 时检查
                 ▼
           ┌───────────────┐
           │ rotateFile    │
           │ 文件切换判断     │
           └───────┬───────┘
                   │创建新文件 + symlink
                   ▼
          ┌────────────────────────┐
          │ AsyncWriter（包装）      │
          │缓冲 / 异步落盘 / 优化IO    │
          └────────────────────────┘
```

---

---

## 🚀 快速使用示例

```go
opt := &RotateOption{
	RotateGenerator: NewRotateGenerator(
		func(ctx context.Context) (RotateInfo, error) {
			t := time.Now().Format("200601021504")
			return RotateInfo{
				Symlink:  "app.log",
				Filename: "app.log",
				FilePath: "app.log." + t,
                SuffixExpr :  regexp.MustCompile(`^\.[0-9]{12}$`),
			}, nil
		},
		time.Minute,
	),
	MaxFileNum: 7,

	NewWriter: func(ctx context.Context, w io.Writer) (AsyncWriter, error) {
		return NewAsyncBufferedWriter(w), nil
	},
	FlushDuration: time.Second,
    CheckDuration: time.Second,
}
opt.RotateGenerator.Start(context.TODO())

w, err := NewRotateFile(opt)
if err != nil {
	panic(err)
}

defer w.Close()

for i := 0; i < 10000; i++ {
	_, _ = w.Write([]byte("hello rotate\n"))
}
```

---

---

## 🔄 自动切换文件时行为

在新周期触发 rotate 时：

1. Flush 当前缓冲数据
2. Close 当前文件句柄
3. 创建新文件
4. 重新 NewWriter
5. 更新软链
6. 清理历史文件

这一过程对调用者透明。

---

---

## 🧠 异步写入机制说明

典型 AsyncWriter 能实现：

* Buffered Write
* 聚合写-AIO
* 自适应写推送
* 全链路 flush 可控

落盘触发方式：

* 队列空 → 自动 flush
* flush ticker → 强制 flush
* close 时 → 优雅 flush

无数据丢失风险。

---

---

## 🧹 文件清理策略说明

指定：

```go
MaxFileNum = 7
```

系统会：

* 收集所有 `{Filename}.*` 真实文件
* 排除软链
* 按修改时间排序
* 超出数量部分删除

不会误删软链文件不会误删非日志文件

---

---

## 🧯 错误处理机制

遇到错误：

* 新文件打开失败 → 继续写旧文件
* symlink 创建失败 → 日志仍可写
* flush 失败 → 不影响后续写入
* generator 出错 → 使用上一次可用值

保证日志不丢失

---

---

## 🧲 线程安全保证

* Write 会持有锁写入
* Flush、Rotate 期间不会并发写
* Latest Rotation 数据安全读取

注：锁粒度为 Writer 切换级别，吞吐性能依赖 AsyncWriter 性能。

---

---

## 🧠 最佳实践建议

| 建议                    | 原因           |
| --------------------- | ------------ |
| FlushDuration ≤ 300ms | 磁盘写入延迟可控     |
| 使用 SSD 或 NVMe         | 更稳定的写延时      |
| 使用软链追踪日志文件            | 运维友好         |
| MaxFileNum 控制 5~15    | 空间稳定         |
| 切分周期避免小于 30s          | 频繁 Close 性能差 |

---

---

## 🔌 扩展方案

该组件允许扩展：

### 📌 自定义切割策略

```go
每小时切割
每天 00:00 切割
按文件大小切割
手动触发切割
按远端通知切割
```

### 📌 自定义 Writer

例如：

```go
func NewWriter(ctx context.Context, w io.Writer) (AsyncWriter, error) {
	return logCompressionWriter(w), nil
}
```

可以：

* gzip 压缩写入
* 按行批量写入
* metrics 包装

---

# License

MIT
